...
object Day12 extends ProvidedInput with App {
  ...
  def plots(grid: Grid): List[List[Plot]] = {
    val groups: List[List[Plot]] = Nil
    val visited: Set[Position] = Set.empty[Position]

    def collectGroup(nodes: List[Node[Char]], visited: Set[Plot]): List[Plot] = {
      if(nodes.isEmpty){
        visited.toList
      } else {
    ...
    val plots = grid.foldLeft((groups, visited)){ case ((g, v), (_, n)) => {
  ...
  def countFences(v: List[model.Vector], d: Direction): Int = {
    val comp: (Vector, Vector) => Boolean ={ case (last, n) =>  d match {
    ...
    sorted match {
      case Nil => 0
      case head :: next => {
        next.foldLeft((head, 1)){ case ((last, c), n) => {
          if(comp(last, n)){
            (n, c)
          } else {
            (n, c + 1)
  ...
  def bulkCost(region: Region) = {
    region.map(p => {
      ...
      val fences = p
        ...
        .groupBy(_.direction)
        .map{ case (d, v) => countFences(v, d)}.sum
        area * fences
    }).sum
  }
